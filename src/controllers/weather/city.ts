...
export const weatherCityScene: WizardScene<TelegrafContext> =
  new Scenes.WizardScene(
    ...
    async ctx => {
      ...
      try {
        const inputCity = (ctx.session.weather.changeCity = capitalize(
          getUserMessage(ctx),
        ));
        ...
        await ctx.replyWithHTML(ctx.i18n.t('weather.info', weatherInfo));
        ...
        noteButton &&
          (await ctx.replyWithHTML(
            ctx.i18n.t('weather.followTitle'),
            Markup.inlineKeyboard([noteButton]),
          ));
        return ctx.wizard.next();
      } catch {
        throw new ValidationError(ctx.i18n.t('error.city'));
