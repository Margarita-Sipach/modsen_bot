...
export class Task {
  ...
  async delete(userId: number) {
    ...
    await UserModel.findByIdAndUpdate(userId, { $pull: { tasks: this.id } });

    this.stopCron();
  }

  async complete() {
    await TaskModel.findByIdAndUpdate(this.id, { status: true });
    this.stopCron();
  }

  async incomplete() {
    await TaskModel.findByIdAndUpdate(this.id, { status: false });
    this.stopCron();
  }
